...
__created__ = "2016"
__scriptVersion__ = "8c"
__lastModified__ = "2020-01-30 17:29 CST"
...
class ObjectSurface(PathOp.ObjectOp):
    ...
    def initOperation(self, obj):
        ...
        obj.addProperty("App::PropertyVectorDistance", "CircularCenterCustom", "Surface", QtCore.QT_TRANSLATE_NOOP("PathOp", "The start point of this path"))
        obj.addProperty("App::PropertyEnumeration", "CircularCenterAt", "Surface", QtCore.QT_TRANSLATE_NOOP("PathOp", "Make True, if specifying a Start Point"))
    ...
    def setEditorProperties(self, obj):
        ...
        if obj.Algorithm == 'OCL Dropcutter':
            ...
            obj.setEditorMode('RespectBoundary', 0)
        else:
            obj.setEditorMode('DropCutterDir', 2)
            ...
            obj.setEditorMode('RespectBoundary', 2)
    ...
    def opExecute(self, obj):
        ...
        if obj.Algorithm == 'OCL Waterline':
            ...
            preRB = obj.RespectBoundary
            obj.CutPattern = 'Line'
            obj.CutPatternAngle = 0.0
            obj.RespectBoundary = False
    ...
    def _makeSafeSTL(self, JOB, obj, Mdl, deflection, voidShapes):
        ...
        if voidShapes is not False:
            ...
            void = FreeCAD.ActiveDocument.addObject("Part::Feature", "safeVoids")
            void.Shape = wstShp
            void.recompute()
            void.purgeTouched()
            tmpObjs.append(void)
    ...
    def _processCutAreas(self, JOB, obj, mdlIdx, faceShapes, voidShapes):
        PathLog.debug('_processCutAreas(self, JOB, obj, mdlIdx, faceShapes, voidShapes)')
        final = list()
        ...
        # Save initial value for restoration later.
        if obj.Algorithm == 'OCL Waterline':
            preHMF = obj.HandleMultipleFeatures
            obj.HandleMultipleFeatures = 'Individually'
        '''
        ...
        if obj.HandleMultipleFeatures == 'Collectively':
            ...
            if obj.ScanType == 'Planar':
                final = self._processPlanarOp(JOB, obj, mdlIdx, COMP)
            elif obj.ScanType == 'Rotational':
                final = self._processRotationalOp(obj, base, COMP)
            ...
            for cas in faceShapes:
                ...
                if obj.Algorithm == 'OCL Waterline':
                    final.append(Path.Command('G0', {'Z': obj.ClearanceHeight.Value, 'F': self.vertRapid}))
                    final.extend(self._waterlineOp(JOB, obj, mdlIdx, COMP))  # independent method set for Waterline
                elif obj.ScanType == 'Planar':
                    final.extend(self._processPlanarOp(JOB, obj, mdlIdx, COMP))
                elif obj.ScanType == 'Rotational':
        ...
        # If algorithm is `Waterline`, restore initial property values
        if obj.Algorithm == 'OCL Waterline':
            obj.HandleMultipleFeatures = preHMF
        '''

        return final

    def _processPlanarOp(self, JOB, obj, mdlIdx, compoundFaces=None):
        PathLog.debug('_processPlanarOp(self, JOB, obj, mdlIdx, compoundFaces=None)')
        initIdx = 0.0
        final = list()
        ...
        if obj.LayerMode == 'Single-pass':
            final = self._planarDropCutSingle(JOB, obj, mdlIdx, compoundFaces)
        elif obj.LayerMode == 'Multi-pass':
            final = self._planarDropCutMulti(JOB, obj, mdlIdx, compoundFaces)
    ...
    def _planarDropCutSingle(self, JOB, obj, mdlIdx, compoundFaces=None):
        PathLog.debug('_planarDropCutSingle(self, JOB, obj, mdlIdx, compoundFaces=None)')
        GCODE = list()
        ...
        fullPDC = self._planarGetPDC(self.safeSTLs[mdlIdx], depthparams[lenDP - 1], obj.SampleInterval.Value)
        ...
        # Recall the CenterOfMass used for Circular cut pattern
        ...
        # Post process SCANS and identify collinear/co-radial lines/arcs
        # ... Breaks are inserted for collinear/co-radial lines/arcs.
        COLIN = list()
        if obj.CutPattern == 'Line':
            COLIN = self._postProcessLineScan(SCANS, lenScans)
        if obj.CutPattern == 'ZigZag':
            (NEWSCANS, COLINEAR) = self._postProcessZigzagScan(SCANS, lenScans)
            SCANS = NEWSCANS
            COLIN = COLINEAR
        elif obj.CutPattern == 'Circular' or obj.CutPattern == 'CircularZigZag':
            COLIN = self._postProcessCircularScan(SCANS, lenScans)
    ...
    def _planarDropCutMulti(self, JOB, obj, mdlIdx, compoundFaces=None):
        GCODE = list()
    ...
    def _postProcessZigzagScan(self, SCANS, lenScans):
        COLIN = list()
        NEWSCANS = list()
        InLine = list()
        ...
        return (NEWSCANS, COLIN)

    def _postProcessLineScan(self, SCANS, lenScans):
        COLIN = list()
        prvFirst = FreeCAD.Vector(SCANS[0][0].x + 5, SCANS[0][0].y - 9, 3.0)
        for ln in range(0, lenScans):
            LN = SCANS[ln]
    ...
    def _postProcessCircularScan(self, SCANS, lenScans):
    ...
    def _planarGetLineSet(self, obj, base, subShp=None):
        ...
        if obj.CutPattern in ['ZigZag', 'Line']:
            ...
            if obj.CircularCenterAt == 'CenterOfMass':
                cntr.Base = COM  # Use center of Mass
            elif obj.CircularCenterAt == 'CenterOfBoundBox':
                cntr.Base = ENV.Shape.BoundBox.Center  
            elif obj.CircularCenterAt == 'XminYmin':
                cntr.Base = FreeCAD.Vector(ENV.Shape.BoundBox.XMin, ENV.Shape.BoundBox.YMin, 0.0)
            elif obj.CircularCenterAt == 'Custom':
                newCent = FreeCAD.Vector(obj.CircularCenterCustom.y, obj.CircularCenterCustom.x, 0.0)
                cntr.Base = newCent
            ...
            if obj.CircularCenterAt != 'CenterOfMass':
                ...
                for c in range(0, 4):
                    ...
                    if dist > dMax:
                        dMax = dist
                lineLen = dMax + (2 * self.cutter.getDiameter())  # Line length to span boundbox diag with 2x cutter diameter extra on each end
                cutPasses = math.ceil(lineLen / self.cutOut) + 1  # Number of lines(passes) required to cover lineLen
            
            # Update current CircularCenter
    ...
    def _waterlineOp(self, JOB, obj, mdlIdx, subShp=None):
        ...
        '''
        pdc = self._planarGetPDC(self.modelSTLs[mdlIdx], depthparams[lenDP - 1], obj.SampleInterval.Value)
        SCANS = self._planarGetLineScans(obj, pdc, base, subShp)
        lenScans = len(SCANS)
        '''
