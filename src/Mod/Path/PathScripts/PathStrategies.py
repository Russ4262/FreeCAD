...
class StrategyProfileOpenEdge:
    ...
    def __init__(self, edgeList, startPoint, depthparams,
                 horizFeed, vertFeed, endVector, radius,
                 safeHeight, clearanceHeight, direction):
            ...
            for wire in self.edgeList:
                ...
                if True or self.endVector is None:
                    ...
                    if self.direction == 'Conventional':
                        pathParams['start'] = FreeCAD.Vector(vrtxs[lv].X, vrtxs[lv].Y, vrtxs[lv].Z)
                    if not start:
...
class StrategyClearing:
    ...
    def execute(self, includePaths=True):
        ...
        if  self.cutPattern == "Adaptive":
            PathLog.error("Adaptive pattern not available via StrategyClearing class.")
            return False
