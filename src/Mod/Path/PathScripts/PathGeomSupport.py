            ...
            if connected == 1:
                if clean:
                    ...
                    if cycle == 1:
                        ...
                        if self.loop_last_idx > 1 and self._edges_connected(self.loop[0], self.loop[self.loop_last_idx]):
                            # extract list of edges from self.loop data and save
                            self.closed_loops.append([self.edges[lei] for lei in self.loop_edges_indexes])
                        else:
                            self.other_edges.extend(self.loop)
                    elif cycle == 2:
                        # Check if list of edges forms closed loop and save accordingly
                        if self.loop_last_idx > 0 and self._edges_connected(self.loop[0], self.loop[self.loop_last_idx]):
                            # extract list of edges from self.loop data and save
                            # PathLog.debug('saving open wire self.loop_edges_indexes: {}'.format(self.loop_edges_indexes))
                            self.open_wires.append([self.edges[lei] for lei in self.loop_edges_indexes])
                        else:
