...
class Working2DAreas:
    ...
    def _debugMsg(self, msg):
        ...
        if self.isDebug:
            ...
            FreeCAD.Console.PrintMessage(
                "PathTargetBuild.Working2DAreas: " + msg + "\n"
            )
        else:
            PathLog.debug(msg)
    ...
    def _applyBoundaryShape(self):
        ...
        if self.boundaryShape == "BoundBox":
            ...
            perimeterFace2 = getCrossSectionFace(self.baseObj.Shape)
            self.allOuters.append(perimeterFace2)
        elif self.boundaryShape == "Stock" and not self.stockProcessed:
            csFace = getCrossSectionFace(self.stockShape)
...
class Working3DFaces:
    ...
    def __init__(
        ...
        # NEW attributes for improved class version
        self.faceItems = list()
        self.wireItesm = list()
        self.edgeItems = list()
        self.solidItems = list()
    ...
    def _debugMsg(self, msg):
        ...
        if self.isDebug:
            # PathLog.info(msg)
            FreeCAD.Console.PrintMessage(
                "PathBuildTarget.Working3DFaces: " + msg + "\n"
            )
        else:
            PathLog.debug(msg)
    ...
    def _clasifyFace(self, baseObject, sub):
        ...
        face = baseObject.Shape.getElement(sub)
        if not type(face.Surface) == Part.Cylinder and not type(face.Surface) == Part.Cone:
            if isVertical(face):
                self.vert.append(face)
                return False
    ...
    def _processEntireBase(self):
        ...
        if name not in self.fullBaseDict.keys():
            ...
            if self.projectionOnly:
                bcs = getBaseCrossSection(self.baseObj.Shape, self.processInternals)
                if bcs:
                    allFacesFused = fuseShapes(bcs.Faces)
                    self.rawAreas.extend(allFacesFused.Faces)
                    self._addDebugObject(allFacesFused, "allFacesFused")
            else:
    ...
    def _processFaces(self):
        ...
        if self.projectionOnly:
            allFaces = list()
            for f in self.nonVerticalFaces:
                try:
                    csf = getCrossSectionFace(f)
                    allFaces.append(csf)
                except:
                    self._debugMsg("_processFaces() unable to cross-section face")
            if allFaces:
                allFacesFused = fuseShapes(allFaces)
                self.rawAreas.extend(allFacesFused.Faces)
                self._addDebugObject(allFacesFused, "allFacesFused")
            return
    ...
    def _identifyWorkAreas(self):
        ...
        # Second face-combining method attempted
        chf = combineHorizontalFaces(self.rawAreas)
        if chf:
            self._debugMsg("combineHorizontalFaces count: {}".format(len(chf)))
            i = 0
            for f in chf:
    ...
    def getWorkingAreas(self, avoidOverhead=False):
        ...
        This is a ghost method in place unitl a parent class is created for working areas.
        """
        self._debugMsg("getWorkingAreas()")
        self._debugMsg(
            "getWorkingAreas() self.projectionOnly is {}".format(self.projectionOnly)
        )

        # PathLog.track(self.handleMultipleFeatures)
        #################################### Force debug mode for class
        # self.isDebug = True
        #################################### Force showing debug shapes for class
        # self.showDebugShapes = True
        ...
        if self.handleMultipleFeatures == "Collectively":
            for tup in self.baseObjectList:
                ...
                self._applyAvoidOverhead(tup[0])
                self._identifyWorkAreas()
                # self.workingAreas.extend(self.rawAreas)
    ...
    def getWorkingSolids(self, avoidOverhead=False):
        ...
        and Extensions features.
        """
        self._debugMsg("getWorkingSolids()")
        # PathLog.track(self.handleMultipleFeatures)
        #################################### Force debug mode for class
        # self.isDebug = True
        #################################### Force showing debug shapes for class
...
class OpenEdge:
    ...
    def _debugMsg(self, msg):
        ...
        if self.isDebug:
            # PathLog.info(msg)
            FreeCAD.Console.PrintMessage("PathTargetBuild.OpenEdge: " + msg + "\n")
        else:
            PathLog.debug(msg)
