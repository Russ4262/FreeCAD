    ...
    def areaOpShapes(self, obj):
        ...
        if obj.Base:
            ...
            for base in self.model:
                ...
                rawTrimSolids = _extrudeBaseDown(base)
                trimSolids = sorted(rawTrimSolids.Solids, key=lambda s: s.BoundBox.ZMin)
                baseTrimTool = trimSolids[0]
                # Part.show(baseTrimTool, "BaseTrimTool")
                # Part.show(rawRemovalShape, "RawRemovalShape")

                obj.removalshape = rawRemovalShape.cut(baseTrimTool)
                # Part.show(obj.removalshape, "RemovalShape")
