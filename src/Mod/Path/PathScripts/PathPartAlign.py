...
class ObjectPartAlign(PathOp.ObjectOp):
    ...
    def initOperation(self, obj):
        ...
        obj.addProperty('App::PropertyBool', 'InvertDirection', 'Alignment', QtCore.QT_TRANSLATE_NOOP('PathPartAlign', ''))
        obj.AlignmentMode = ['Auto', 'Manual']
        obj.AlignmentType = ['Static', 'Dynamic']

    def opSetDefaultValues(self, obj, job):
        '''opSetDefaultValues(obj, job) ... initialize defaults'''
        obj.InvertDirection = False
    ...
    def opExecute(self, obj):
            ...
            for b in obj.Base:
                for sub in b[1]:
                    ...
                    if len(edge.Vertexes) == 2:
                        line_edge_tuples.append((sub, edge))
                    else:
        ...
        connected_edge_sets = self.find_connected_edges(line_edge_tuples)

        eCnt = len(line_edge_tuples)
        alignmentPoints = []
        if eCnt > 1:
            if eCnt % 2 == 0:
                pCnt = int(eCnt / 2)
            ...
            for i in range(0, pCnt):
                (sub0, edg0) = line_edge_tuples.pop(0)
                ev0 = edg0.Vertexes[0]
                ev1 = edg0.Vertexes[1]
                ...
                for ei in range(0, len(line_edge_tuples)):
                    (sub1, edg1) = line_edge_tuples[ei]
                    mtch = ei
                    cv0 = edg1.Vertexes[0]
                    cv1 = edg1.Vertexes[1]
                ...
                if cvi != 2:
                    ...
                    if p == 4:
                        # Tuple is (target bit point tangent to work piece, common vertex point of selected, connected edges)
    ...
    def find_connected_edges(self, line_edge_tuples):
        connected_edge_sets = list()
        return connected_edge_sets
