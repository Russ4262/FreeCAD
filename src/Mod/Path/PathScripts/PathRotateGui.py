...
import FreeCAD
import FreeCADGui
import PathScripts.PathRotate as PathRotate
...
__title__ = "Path Rotate Operation UI"
__author__ = "sliptonic (Brad Collette)"
__url__ = "http://www.freecadweb.org"
__doc__ = "Rotate operation page controller and command implementation."
...
class TaskPanelOpPage(PathOpGui.TaskPanelPage):
    ...
    def getFields(self, obj):
        ...
        '''PathGui.updateInputFieldFromComboBox(obj, 'EnableRotation', self.form.enableRotation)
        PathGui.updateInputFieldFromComboBox(obj, 'IndexingMode', self.form.indexingMode)'''
        ...
        '''
        if obj.IndexingMode != str(self.form.indexingMode.currentText()):
            obj.IndexingMode = str(self.form.indexingMode.currentText())

        if obj.ReverseDirection != self.form.reverseDirection.isChecked():
            obj.ReverseDirection = self.form.reverseDirection.isChecked()

        if obj.AltDepthCalc != self.form.altDepthCalc.isChecked():
            obj.AltDepthCalc = self.form.altDepthCalc.isChecked()
        '''
...
Command = PathOpGui.SetupOperation('Rotate',
        PathRotate.Create,
        TaskPanelOpPage,
        'Path-Rotate',
        QtCore.QT_TRANSLATE_NOOP("Rotate", "Rotate"),
        QtCore.QT_TRANSLATE_NOOP("Rotate", "Rotate Operation: rotate the model"),
