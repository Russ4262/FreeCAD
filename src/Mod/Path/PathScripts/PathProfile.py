...
class ObjectProfile(PathOp.ObjectOp):
        ...
        if shapes:
            ...
            for shp, __, __ in shapes:
                if isinstance(shp, list):
                    workingShapes.extend(shp)
                else:
                    workingShapes.append(shp)
            obj.WorkingShape = Part.makeCompound(workingShapes)

        return sims

    def shapeIdentification(self, obj, isPreview=False):
        """shapeIdentification(obj) ... returns envelope for all base shapes or wires for Arch.Panels."""
        PathLog.track()
...
def SetupProperties():
    ...
    setup.extend([tup[1] for tup in ObjectProfile.opPropertyDefinitions(None)])
    return setup
