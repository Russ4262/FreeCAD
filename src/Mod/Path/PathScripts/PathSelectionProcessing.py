...
__doc__ = "Collection of classes and functions used to process and refine user selections."
__contributors__ = ""
...
def combineHorizontalFaces(faces):
    ...
    if outer:
        ...
        if innerFaces:
            ...
            # Debugging
            Part.show(innerComp)
            FreeCAD.ActiveDocument.ActiveObject.Label = "innerComp"
            FreeCAD.ActiveDocument.ActiveObject.purgeTouched()

            Part.show(outerComp)
            FreeCAD.ActiveDocument.ActiveObject.Label = "outerComp"
            FreeCAD.ActiveDocument.ActiveObject.purgeTouched()
            '''
    ...
    for wire in vWires:
        ...
        if w.isClosed():
            ...
            if isRoughly(face.Area, 0):
                PathLog.debug(translate('PathPocket', 'Vertical face(s) do not form a loop - ignoring'))
            else:
...
def isVerticalExtrusionFace(face):
    ...
    if hasattr(extr, "Volume"):
        ...
        if extr.Volume < face.Area * Tolerance:
            PathLog.debug('isVerticalExtrusionFace() Check if extruded face is vertical')
            return True
        else:
...
def get3DEnvelope(baseShape, faceList, envTargetHeight):
    ...
    # clean = solid.removeSplitter()
    # Part.show(clean)
    # FreeCAD.ActiveDocument.ActiveObject.Label = "clean"
    # FreeCAD.ActiveDocument.ActiveObject.purgeTouched()
    ...
    targetShape = solid.cut(topBox)  # was `clean.cut(topBox)`, but removeSplitter() is causing issues
...
def getBaseCrossSection(baseShape, includeInternals=True, isDebug=False):
    ...
    topBox.translate(FreeCAD.Vector(bsBB.XMin - 5.0, bsBB.YMin - 5.0, bsBB.ZMin + extrLen2))
    ...
    if includeInternals:
        ...
        for f in cut.Faces:
            fBB = f.BoundBox
            if isRoughly(fBB.ZMin, 0.0) and \
                isRoughly(fBB.XLength, xLen) and \
                isRoughly(fBB.YLength, yLen):
...
def splitClosedWireAtTwoVertexes(closedWire, vertA, vertB, tolerance):
    ...
    pnt_A = vertA.Point
    pnt_B = vertB.Point
    # Debugging - show target verticies, vertA and vertB, with vertical lines
    # lnA = Part.makeLine(pnt_A, FreeCAD.Vector(vertA.X, vertA.Y, vertA.Z + 20.0))
    # lnB = Part.makeLine(pnt_B, FreeCAD.Vector(vertB.X, vertB.Y, vertB.Z + 20.0))
    # Part.show(lnA)
    # Part.show(lnB)
...
def getOverheadRegionsAboveHeight(baseShape, height, isDebug=False):
    ...
    bottomBox.translate(FreeCAD.Vector(bsBB.XMin - 1.0, bsBB.YMin - 1.0, bsBB.ZMin - extra))
    overheadShape = baseShape.cut(bottomBox)  # base shape cut off bottom portion at height
    ...
    overheadShape.translate(FreeCAD.Vector(0.0, 0.0, 0.0 - overheadShape.BoundBox.ZMin))  # Place bottom at zero
    ...
    # DO NOT REMOVE
    # This active document recompute might be required to force internal shape recompute/update of targetShape
    # This does throw a warning message in the Report View about recursive recomputes,
    # but it achieves the necessary result.
...
def getRegionsBelowHeight(baseShape, height, isDebug=False):
    ...
    # Relocate bottom of overhead shape to Z=0.0
    lowerShape.translate(FreeCAD.Vector(0.0, 0.0, -10.0 - lowerShape.BoundBox.ZMax))  # Place top at -10
    ...
    # DO NOT REMOVE
    # This Part.show() and subsequent deletion is required to force internal shape recompute/update of targetShape
    Part.show(targetShape)
    name = FreeCAD.ActiveDocument.ActiveObject.Name
    FreeCAD.ActiveDocument.ActiveObject.purgeTouched()
    FreeCAD.ActiveDocument.removeObject(name)
...
class Working2DAreas:
    ...
    def __init__(self,
        ...
        self.overheadRegionsDict = dict()  # Save overhead regions by base.Name for external class access
    ...
    def _debugMsg(self, msg):
        ...
        if self.isDebug:
            # PathLog.info(msg)
            FreeCAD.Console.PrintMessage("SelectionProcessing.Working2DAreas: " + msg + "\n")
        else:
            PathLog.debug(msg)
                ...
                if sub.startswith('Face'):
                    ...
                    if sub not in self.avoidFeatures:
                        if not self._clasifyFace(base, sub):
                            msg = translate('PathPocket', 'Pocket does not support shape %s.%s') % (base.Label, sub)
                            PathLog.error('getWorkingAreas(): ' + msg)
                    else:
                        msg = translate('PathPocket', 'Avoiding %s.%s') % (base.Label, sub)
        ...
        if wires:
            for w in wires:
                if w.isClosed():
                    ...
                    self.openWireTups.append((self.baseObj, w))  # Later processing of open edges in Profile module
        else:
        ...
        if wires:
            for w in wires:
                if w.isClosed():
                    ...
                    self._debugMsg('Working2DAreas: Flattened wire is not a closed wire.')
                    rawOpenEdgeBaseTups.append((self.baseObj, w))  # Later processing of open edges in Profile module
        else:
    ...
    def _categorizeFaces(self, faceList, sourceCompound):
        ...
        if trueFaces:
            ...
            for tf in trueFaces:
                tf.translate(FreeCAD.Vector(0.0, 0.0, sourceCompound.BoundBox.ZMax - tf.BoundBox.ZMin))
                self.baseFacesDict[self.baseObj.Name].append(tf)
        elif faces:
            self.horiz.extend(faces)
            for f in faces:
                f.translate(FreeCAD.Vector(0.0, 0.0, sourceCompound.BoundBox.ZMax - f.BoundBox.ZMin))
    ...
    def _processVertFaces(self):
        ...
        if horizFaces:
            ...
            for f in horizFaces:
                f.translate(FreeCAD.Vector(0.0, 0.0, vert.BoundBox.ZMin - f.BoundBox.ZMin))
            self.horiz.extend(horizFaces)
            return

        self._debugMsg('Working2DAreas: Attempting to convert non-looped vertical faces to open edges.')
        lowZMax = min([f.BoundBox.ZMax for f in self.vert])  # determine lowest ZMax in face list
        ...
        for f in self.vert:
            ...
            if wire:
                ...
                self._debugMsg("flattenVerticalFace() failed to return edge(s) for a vertical face.")
    ...
    def _processHorizFaces(self):
        ...
        self._debugMsg('_processHorizFaces({})'.format(len(self.horiz) if len(self.horiz) > 0 else 0))
    ...
    def _getCrossSectionFace(self, shape):
        ...
        tmpDepthParams = PathUtils.depth_params(
            clearance_height=max + 5.0, safe_height=max + 3.0,
            start_depth=max, step_down=5.0,
            z_finish_step=0.0, final_depth=shapeBB.ZMin,
            user_depths=None)
    ...
    def _processExtensions(self):
        ...
        for ext in self.extensions:
            ...
            if not ext.avoid and \
              ext.obj.Name == self.baseObj.Name and \
              ext.feature in self.subs[self.baseObj.Name]:
    ...
    def _applyAvoidOverhead(self, base):
        ...
        if not self.avoidOverhead or (not self.workingAreas and not self.workingHoles) or self.disableOverheadCheck:
            return
        ...
        if self.finalDepth < self.zMin:
            ...
            # PathLog.warning(msg)
            msg2 = translate('PathGeom', 'High point of selection: {} mm.'.format(round(self.zMin, 6)))
            PathLog.debug(msg2)
        ...
        self._addDebugObject(Part.makeCompound(self.workingAreas), objName='pre_overheadRegions_workingAreas')
        self._addDebugObject(overhead, objName='{}_overheadRegions_{}'.format(base.Name, height))
        ...
        if self.processInternals and self.workingHoles:
            self._debugMsg("pre-overhead holes count: {}".format(len(self.workingHoles)))
    ...
    def getWorkingAreas(self, avoidOverhead=False):
        ...
        if self.workingAreas:
            if self.isDebug:
                self._debugMsg("returning self.workingAreas")
        ...
        if self.handleMultipleFeatures == 'Individually' and self.allVert and not self.workingAreas:
            PathLog.info(translate('PathSelectionProcessing', 'Verify `HandleMultipleFeatures` property.'))
    ...
    def getHoleAreas(self):
        ...
        if self.processInternals and self.workingHoles:
            ...
            if fusedHoles:
                ...
                self._debugMsg("self.workingHoles count: {}".format(len(self.workingHoles)))
                return self.workingHoles

        return None

    def getOpenEdges(self, toolRadius, offsetRadius, useToolComp, jobTolerance, jobLabel='Job'):
        ...
        for (base, wire) in self.rawOpenEdgeBaseTups:
            ...
            oe.showDebugShapes = self.showDebugShapes  # Transfer show debug shapes status
    ...
    def getOverheadRegions_3D(self, base, height):
        self._debugMsg('getOverheadRegions_3D({}, height={}mm)'.format(base.Name, height))
...
class Working3DFaces:
    ...
    def __init__(self,
        ...
        for base, subsList in baseObjectList:
            self.baseFacesDict[base.Name] = [base.Shape.getElement(sub) for sub in subsList if sub.startswith('Face')]
    ...
    def _debugMsg(self, msg):
        ...
        if self.isDebug:
            # PathLog.info(msg)
            FreeCAD.Console.PrintMessage("SelectionProcessing.Working3DFaces: " + msg + "\n")
        else:
            PathLog.debug(msg)
        ...
        for base, subs in self.baseSubsTups:
            ...
            for sub in subs:
                ...
                if sub.startswith('Face'):
                    ...
                    msgNoSupport = translate('PathPocket', 'Pocket does not support shape %s.%s') % (base.Label, sub)
            ...
            if subCnt == 0:
                ...
                self._debugMsg("_processBaseSubsList() {}: {} subs".format(base.Name, subCnt))
                self._processEdges()
                self._processFaces()
    ...
    def _processSolid(self, solidShape):
        ...
        box.translate(FreeCAD.Vector(bsBB.XMin - 5.0, bsBB.YMin - 5.0, -1.0 * (extent + 5.0)))
        surfaceBase = fused.cut(box)
        # Part.show(surfaceBase)
    ...
    def _processFaces(self):
        ...
        self._addDebugObject(Part.makeCompound(self.nonVerticalFaces), 'nonVerticalFaces')
    ...
    def _trimExtrusion(self, extrusion, height):
        ...
        box = Part.makeBox(extBB.XLength + 2.0, 
                           extBB.YLength + 2.0,
                           math.floor(extBB.ZLength))
        move = FreeCAD.Vector(extBB.XMin - 1.0,
                              extBB.YMin - 1.0,
                              height)
        box.translate(move)
    ...
    def _applyAvoidOverhead(self, base):
        ...
        self._addDebugObject(overheadRegions, objName='{}_overheadRegions'.format(base.Name))
...
class OpenEdge:
    ...
    def _getCutAreaCrossSection(self, origWire, flatWire):
        ...
        if isRoughly(wBB.ZLength, 0.0):
            ...
            thickCS = getThickCrossSection(self.baseShape, wBB.ZMin, wBB.ZMax, isDebug=False)
        ...
        if self.useOtherSide:  # alternate Inside/Outside application in `_extractPathWire()` in progress...
        ...
        self.pathStops.translate(FreeCAD.Vector(0, 0, 0.0 - self.pathStops.BoundBox.ZMin))
        self.otherPathStops.translate(FreeCAD.Vector(0, 0, 0.0 - self.otherPathStops.BoundBox.ZMin))
        ...
        if wi > 0:  # and isInterior is False:
            self._debugMsg('Multiple wires in cut area. First choice is not 0. Testing.')
            ...
            if testArea.Area < minArea:
                self._debugMsg('offset area is less than minArea of {}.'.format(minArea))
                self._debugMsg('Using wire index {}.'.format(wi - 1))
                pWire = Part.Wire(Part.__sortEdges__(workShp.Wires[wi - 1].Edges))
                pfcShp = Part.Face(pWire)
                pfcShp.translate(FreeCAD.Vector(0, 0, fdv - workShp.BoundBox.ZMin))
    ...
    def _checkTagIntersection(self, iTAG, eTAG, cutSide, tstObj, begInt=None, begExt=None):
    ...
    def _extractPathWire(self, flatWire, cutShp):
        ...
        (part0, part1) = splitClosedWireAtTwoVertexes(mainWire,
    ...
    def _makeStop(self, sType, pA, pB, lbl):
        ...
        if self.useToolComp is True or (self.useToolComp is False and self.offsetExtra != 0):
    ...
    def getOpenEdges(self):
        ...
        if self.jobTolerance == 0.0:
            ...
            msg += translate('PathSelectionProcessing', 'Please set to an acceptable value greater than zero.')
            PathLog.error(msg)
            return openEdges
        ...
        self.sortedFlatWire = Part.Wire(Part.__sortEdges__(flatWire.Wires[0].Edges))  # complex selections need edges sorted
...
class Item:
    def __init__(self, base, sub, siblingList=None):
        ...
        self.sub
        self.itemType = 'Base'
        self.siblingList = siblingList if siblingList and isinstance(siblingList, list) else None
        self.subGoupKey = ''
        self.shape = None
        self.vertexes = list()
        ...
        if sub is not None and isinstance(sub, str):
            ...
            self.flatVertexes = [FreeCAD.Vector(v.X, v.Y, 0.0) for v in self.shape.Vertexes]
