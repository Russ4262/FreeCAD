...
FeatureExtensions = LazyLoader('PathScripts.PathFeatureExtensions', globals(), 'PathScripts.PathFeatureExtensions')
...
class TaskPanel(object):
    ...
    def __init__(self, obj, deleteOnReject, opPage, selectionFactory, parent=None):
        ...
        self._addVisualizeSelectionButton(opPage)  # Add `Visualize` button to standard buttons (repurposing `Help` button)
    ...
    def _initOpAndFeaturePages(self, opPage, obj):
        ...
        if PathOp.FeatureExtensions & features:
    ...
    def _addVisualizeSelectionButton(self, opPage):
        '''_addVisualizeSelectionButton() ... Add standard Help button, and repurpose for visualizing selections'''
        objName = self.parent.OpName
        for n in ["Profile", "Pocket", "Adaptive", "Waterline"]:
            if objName.startswith(n):
                opPage.form.visualizeButton = QtGui.QPushButton()
                opPage.form.visualizeButton.setCheckable(True)
                opPage.form.visualizeButton.setText("Visualize")
                opPage.form.layout().addWidget(opPage.form.visualizeButton)
                break
    ...
    def preCleanup(self):
        ...
        if page.workingShapeList and self.switch:
            for (__, __, ds) in page.workingShapeList:
...
class TaskPanelPage(object):
    ...
    def __init__(self, obj, features):
        ...
        self.workingShapeList = list()  # stores tuples of "(label, coinObj)" for removal shape preview
...
class TaskPanelDepthsPage(TaskPanelPage):
    ...
    def __init__(self, obj, features):
        ...
        self.finishDepth = None
        self.stepDown = None
        self.title = 'Depths and Heights'
        self.OpIcon = ":/icons/Path_Depths.svg"
        self.setIcon(self.OpIcon)
        self.refImagePath = "{}Mod/Path/Images/Ops/{}".format(FreeCAD.getHomePath(), 'Path-DepthsAndHeights.gif')  # pylint: disable=attribute-defined-outside-init
        self.refImage = QtGui.QPixmap(self.refImagePath)  # pylint: disable=attribute-defined-outside-init
    ...
    def getForm(self):
        return FreeCADGui.PySideUic.loadUi(":/panels/PageDepthsHeightsEdit.ui")
    ...
    def getTitle(self, obj):
        return translate("PathOp", "Depths and Heights")
    ...
    def pageUpdateData(self, obj, prop):
        if prop in ['StartDepth', 'FinalDepth', 'StepDown', 'FinishDepth', 'SafeHeight', 'ClearanceHeight']:
            self.setFields(obj)
...
class TaskPanelExtensionPage(TaskPanelPage):
    ...
    def _initializeExtensions(self, obj):
        ...
        if len(obj.ExtensionFeature) > 0:  # latter test loads pre-existing extensions (editing of existing operation)
            hasEdges = False
            hasSpecial = False
            ...
            for base in self.obj.Base:
                ...
                if self._allEdges(base[1]):
                    for sub in sorted(base[1]):
                        rowItem = self.createItemForBaseModel(base[0], sub, edges, extensions)
                        if rowItem:
    ...
    def _getUsePerimeterState(self):
        ...
        if hasattr(opf, 'processPerimeter') and hasattr(opf, 'processHoles') and hasattr(opf, 'processCircles'):
            PathLog.debug("Found processPerimeter ")
            self.usePerimeter = opf.processPerimeter.isChecked() and not opf.processHoles.isChecked() and not opf.processCircles.isChecked()
            return
