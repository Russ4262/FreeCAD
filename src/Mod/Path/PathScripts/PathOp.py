...
FeatureFixedRotation = 0x8000     # Feature Working Plane
...
class ObjectOp(object):
    ...
    def getFaceNormAndSurf(self, face):
        ...
        if n is not None and s is not None:
            ...
            PathLog.error("PathAreaOp.getFaceNormAndSurf()")
            return False
    ...
    def orientModelToFixedIndex(self, obj):
        ...
        if obj.FixedIndexReference != 'None':
            if obj.EnableRotation == 'Off':
                PathLog.error(translate("Path", "To utilize the Fixed Index feature, you must 'Enable Rotation' for the Job."))
                obj.FixedIndexReference = 'None'
                return fail
            else:
        ...
        if obj.FixedIndexReference == 'Previous':
            (activeFixedIndex, rotAng, axisVect, millAxis) = self.orientModelToPrevious(Job, obj)
        elif obj.FixedIndexReference == 'Custom Plane':
            PathLog.error("Custom Plane for Work Plane is unavailable (incomplete code).")
            # obj.CustomPlaneAngle = 0.0
            # obj.CustomPlaneAxis = FreeCAD.Vector(0, 0, 0)
            # obj.CustomPlaneBase = FreeCAD.Vector(0, 0, 0)
            ...
            if obj.FaceOnReferenceObject > len(refObj.Shape.Faces):
                msg = "Reference object, {}, only contains {} faces.".format(objName, len(refObj.Shape.Faces))
        ...
        if analyzeFace is True:
            (activeFixedIndex, rotAng, axisVect, millAxis) = self.analyzeRefFace(obj, refFace)

        if activeFixedIndex is True:
            rotCmds = self.applyFixedIndex(Job, obj, rotAng, axisVect, millAxis)
            return (True, rotCmds)  # (isRotationActive, rotationGcodeCommands)
        else:
            return fail

    def orientModelToPrevious(self, obj):
        if len(Job.Operations.Group) > 1:
            ...
            if hasattr(prevOp, 'FixedIndexReference'):
                if prevOp.FixedIndexReference == 'None':
                    ...
                    PathLog.debug("prevOp working plane: rotAng: {};  axisVect: {};  millAxis: {}.".format(rotAng, axisVect, millAxis))
    ...
    def analyzeRefFace(self, obj, refFace):
        # Determine Surface.Axis and normalAt(0,0) values of reference face
        (norm, surf) = self.getFaceNormAndSurf(refFace)

        if surf is not None and norm is not None:
            (rtn, rotAng, cAxis, praInfo) = self.faceRotationAnalysis(obj.EnableRotation, norm, surf, obj.OppositeIndexAngle)
            if rtn is True:
                if cAxis == 'X':
                    millAxis = 'A'
    ...
    def applyFixedIndex(self, Job, obj, rotAng, axisVect, millAxis):
        ...
        self.resetModelToInitPlacement(obj, True)
        rotate(Job.Model.Group, rotAng, center=FreeCAD.Vector(0, 0, 0), axis=axisVect, copy=False)
        if Job.Stock:
            rotate([Job.Stock], rotAng, center=FreeCAD.Vector(0, 0, 0), axis=axisVect, copy=False)
    ...
    def resetModelToInitPlacement(self, obj, reset):
        ...
        if len(Job.Model.Group) > 0:
            if reset is True:
                ...
                Job.Stock.Placement.Rotation = FreeCAD.Rotation(Job.Stock.InitAxis, Job.Stock.InitAngle)
                Job.Stock.purgeTouched()
                ...
                if obj.CustomPlaneAngle != 0.0:
                    ...
                    if obj.VerticalIndexOffset == 0.0:
                        self.commandlist.append(Path.Command('G1', {obj.MillIndexAxis: 0.0, 'F': self.axialFeed}))
            elif obj.ResetIndexTo == 'Previous':
                PathLog.error("resetModelToInitPlacement to 'Previous' (incomplete code).")
            elif obj.ResetIndexTo == 'None':
                PathLog.debug("  --Not reseting model placement: resetModelToInitPlacement = 'None'.")
            else:
                PathLog.error("resetModelToInitPlacement()")

    def alignFaceToView(self, refFace):
        '''alignFaceToView(refFace)
