...
class TaskPanelOpPage(PathTaskPanelPage.TaskPanelPage):
    ...
    def registerSignalHandlers(self, obj):
        ...
        self.form.invertDirection.clicked.connect(self._showRotationReference)
    ...
    def _useSelectedModelAndFeature(self):
        ...
        if "Edge" == subShp.ShapeType:
            if subShp.Curve.TypeId not in ["Part::GeomLine", "Part::GeomLineSegment"]:
                FreeCAD.Console.PrintWarning("Edge must be line.\n")
                return
            (rotations, isPlanar) = TargetShape.getRotationToLine(modelName, featName)
        elif "Face" == subShp.ShapeType:
            (rotations, isPlanar) = TargetShape.getRotationToPlanarFace(
                modelName, featName
            )
            if not isPlanar:
                FreeCAD.Console.PrintWarning("Face must be planar.\n")
                return
        else:
            FreeCAD.Console.PrintWarning("Selection must be an edge.\n")
            return
    ...
    def _invertReferenceArrow(self):
        pass
