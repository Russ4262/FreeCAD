...
__title__ = "Path Facing Operation"
__author__ = "russ4262 (Russell Johnson)"
__url__ = "https://www.freecadweb.org"
__doc__ = "Class and implementation of Facing operation."
...
class ObjectFacing(PathOp2.ObjectOp2):
    """Proxy object for Facing operation."""
    ...
    def opPropertyDefinitions(self):
        return ObjectFacing.propDefinitions()

    def opPropertyEnumerations(self):
        return ObjectFacing.propEnumerations()

    def opPropertyDefaults(self, obj, job):
        return ObjectFacing.propDefaults(obj, job)
    ...
    def opExecute(self, obj, getsim=False):  # pylint: disable=arguments-differ
        ...
        # shapes = self.getTargetShape(obj, facesOnly=True)
        ...
        for shape, baseObj, details in shapes:
            ...
            if strategy.executeOCL(baseObj, obj):
        ...
        times = [
            "D %02d  " % (elapsed.days),
            "H %02d  " % (elapsed.seconds / 3600),
            "M %02d  " % (elapsed.seconds / 60 % 60),
            "S %02d" % (elapsed.seconds % 60),
        ]
        timeText = "Elapsed operation time: "
        prnt = False
        for t in times:
            if "00" not in t:
                prnt = True
    ...
    def getTargetShape_ORIG(self, obj, isPreview=False, facesOnly=False):
        """getTargetShape(obj) ... return shapes representing the solids to be removed."""
        PathLog.track()
        # PathLog.info("getTargetShape()")
        removalShapes = []
        workingAreas = None
        extensions = None
        ...
        pac.isDebug = (
            self.isDebug
        )  #  You can force True here to set debug mode for PathTarget2DArea class
        pac.showDebugShapes = (
            obj.ShowDebugShapes
        )  #  You can force True here to show debug shapes for PathTarget2DArea class
        ...
        if workingAreas:
            if facesOnly:
                removalShapes.extend(
                    [(wa, base, "pathFacing") for base, wa in workingAreas]
                )
            else:
                ...
                removalShapes.extend(
                    [
                        (face.removeSplitter().extrude(extent), base, "pathFacing")
                        for base, face in workingAreas
                    ]
                )
    ...
    def getTargetShape(self, obj, isPreview=False):
        ...
        if obj.TargetShape is not None:
            ...
            baseShapes = obj.TargetShape.Proxy.getTargetGeometry(
                obj.TargetShape, shapeTypes=["2D Area"]
            )
        else:
...
def SetupProperties():
    setup = list()
    setup.extend([tup[1] for tup in ObjectFacing.propDefinitions()])
    setup.extend(
        [
            tup[1]
...
def Create(name, obj=None, parentJob=None):
    """Create(name) ... Creates and returns a Facing operation."""
    if obj is None:
        obj = FreeCAD.ActiveDocument.addObject("Path::FeaturePython", name)
    obj.Proxy = ObjectFacing(obj, name, parentJob)
    return obj
