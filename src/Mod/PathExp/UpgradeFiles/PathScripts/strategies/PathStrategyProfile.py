...
class StrategyProfile:
    ...
    def __init__(
        ...
        self.resume_height = depthparams.safe_height  # obj.SafeHeight.Value
        self.retraction = depthparams.clearance_height  # obj.ClearanceHeight.Value
        ...
        self.radius = self.toolDiameter / 2.0
...
class StrategyProfileOpenEdge:
    ...
    def generateCommands(self):
        ...
        if len(self.edgeList) == 0:
            PathLog.error("StrategyProfileOpenEdge: no edges received")
            return
        ...
        for i in range(0, cntHeights):
            ...
            if i < cntHeights - 1:
                print("PathStrategyProfile.checkpoint_1")
...
def profileFace(
    ...
    resume_height = depthparams.safe_height  # obj.SafeHeight.Value
    retraction = depthparams.clearance_height  # obj.ClearanceHeight.Value

    vertFeed = toolController.VertFeed.Value
    # vertRapid = toolController.VertRapid.Value
    horizFeed = toolController.HorizFeed.Value
    ...
    """generateCommands() ... public function to generate gcode for path area face."""

    loc = FreeCAD.Vector(
        face.BoundBox.Center.x, face.BoundBox.Center.y, face.BoundBox.ZMin
    )
    wp = Part.makeCircle(10, loc)
    ...
    offset = materialAllowance  # 0.0
    if useToolCompensation:
        offset = radius + materialAllowance
    if cutSide == "Inside":
        offset = 0 - offset
    if isHole:
        offset = 0 - offset
    ...
    jointype = ["Round", "Square", "Miter"]
    areaParams["JoinType"] = jointype.index(joinType)

    if joinType == "Miter":
        areaParams["MiterLimit"] = miterLimit

    area.setParams(**areaParams)
    # areaParams = str(area.getParams())

    PathLog.debug("Area with params: {}".format(area.getParams()))

    sections = area.makeSections(mode=0, project=opUseProjection, heights=heights)
    PathLog.debug("sections = %s" % sections)
    shapelist = [sec.getShape() for sec in sections]
    PathLog.debug("shapelist = %s" % shapelist)
    ...
    # Reverse the direction for holes
    if isHole:
        direction = "Climb" if direction == "Conventional" else "Conventional"
    else:
        direction = direction
    ...
    if useToolCompensation:
        pathParams["orientation"] = orientation.index(direction)
    else:
        if direction == "Conventional":
            pathParams["orientation"] = 1
        else:
            pathParams["orientation"] = 0

    pathParams["shapes"] = shapelist
    pathParams["feedrate"] = horizFeed
    pathParams["feedrate_v"] = vertFeed
    pathParams["verbose"] = True
    pathParams["resume_height"] = resume_height
    pathParams["retraction"] = retraction
    pathParams["return_end"] = True
    # Note that emitting preambles between moves breaks some dressups and prevents path optimization on some controllers
    pathParams["preamble"] = False

    # Might need to comment out so as to disable.
    if not opRetractTool:
        pathParams["threshold"] = 2.001 * radius

    if endVector is not None:
        pathParams["start"] = endVector
    elif startPoint:
    ...
    #    {key: value for key, value in pathParams.items() if key != "shapes"}
    # )
    # PathLog.debug("Path with params: {}".format(pathParams))

    (pp, end_vector) = Path.fromShapes(**pathParams)
    # PathLog.debug("pp: {}, end vector: {}".format(pp, end_vector))
    # Part.show(Part.makeCompound(shapelist))

    return Part.makeCompound(shapelist), pp.Commands, end_vector
