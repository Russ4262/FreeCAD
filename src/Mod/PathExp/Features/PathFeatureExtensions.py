...
def _extendEdge(feature, e0, direction, length):
    ...
    return extendWire(feature, Part.Wire([e0]), length)


def _offsetDiscretizedEdge(feature, edge, direction, length):
    # Discretize wire, then offset new wire
    pnts = edge.discretize(Deflection=0.01)
    ln1 = []
    ln2 = []
    p0 = pnts[0]
    for p1 in pnts[1:]:
        ln1.append(Part.makeLine(p0, p1))
        ln2.append(Part.makeLine(p1, p0))
        p0 = p1
    wire1 = Part.Wire(ln1)
    ln2.reverse()
    off = length * direction
    wire2 = Part.Wire(ln2)
    wire2.translate(off)
    e2 = Part.makeLine(wire1.Vertexes[-1].Point, wire2.Vertexes[0].Point)
    e4 = Part.makeLine(wire2.Vertexes[-1].Point, wire1.Vertexes[0].Point)
    wire = Part.Wire(ln1 + [e2] + wire2.Edges + [e4])
    return wire
...
def extendWire(feature, wire, length):
    ...
    PathLog.debug("extendWire() returning None")
    return None
...
class Extension(object):
    ...
    def _getDirectedNormal(self, p0, normal):
        ...
        if not self.rotatedBaseShp.isInside(poffPlus, 0.005, True):
            return normal

        # if not self.obj.Shape.isInside(poffMinus, 0.005, True):
        if not self.rotatedBaseShp.isInside(poffMinus, 0.005, True):
    ...
    def _processRegularEdge(self, feature, edges, sub, length):
        ...
        if Part.Circle == type(edge.Curve):
            ...
            if direction is None:
                PathLog.error("direction is None")
                return None
    ...
    def _processRegularMultiEdges(self, feature, sub, length):
        ...
        trimmedWire = extFace.cut(self.rotatedBaseShp).Wires[0]
