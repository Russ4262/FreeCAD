...
class PathAdaptive(PathOp.ObjectOp):
    ...
    def initOperation(self, obj):
        ...
        obj.addProperty(
            ...
            QT_TRANSLATE_NOOP(
                "App::Property",
                "Set True if solid is an extruded 2D shape.  Set False for 3D Solids, to use dynamic layering.",
            ),
        )
    ...
    def opExecute(self, obj):
        ...
        Should be overwritten by subclasses."""
        _printCurrentDepths(obj, "PathAdaptive.opExecute")
        ...
        depth_params = PathUtils.depth_params(
            ...
            safe_height=obj.SafeHeight.Value,
            start_depth=obj.StartDepth.Value
            - obj.Proxy.job.GeometryTolerance.Value / 4.0,
            step_down=stepDown,
            z_finish_step=finish_step,
            final_depth=obj.FinalDepth.Value
            + obj.Proxy.job.GeometryTolerance.Value / 4.0,
        ...
        if obj.UseProjection:
            ...
            for layerDepth in depth_params.data:
                ...
                self.depth_params = PathUtils.depth_params(
                    clearance_height=obj.ClearanceHeight.Value,
                    safe_height=obj.SafeHeight.Value,
                    start_depth=lastDepth,
                    step_down=stepDown,
                    z_finish_step=0.0,
                    final_depth=layerDepth,
                    user_depths=None,
                )
                # Slice the solid at layer depth
                wires = []
                for i in obj.TargetShape.Shape.slice(
                    FreeCAD.Vector(0, 0, 1), layerDepth
                ):
                    if i.isClosed():
                        wires.append(i)
                # Discretize wires for Adaptive algorithm
                edge_list = list()
                for w in wires:
                    for e in w.Edges:
                        edge_list.append([discretize(e)])
...
def _printCurrentDepths(obj, label):
    print(
...
def _getBottomFacesOfSolid(solid):
    bb = solid.BoundBox
    zMin = bb.ZMin
    return [
        f
...
FreeCAD.Console.PrintMessage("Imported PathAdaptive module from PathExp workbench.\n")
