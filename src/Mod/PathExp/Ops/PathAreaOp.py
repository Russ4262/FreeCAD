...
class ObjectOp(PathOp.ObjectOp):
    ...
    def initOperation(self, obj):
        ...
        obj.addProperty("Part::PropertyPartShape", "PathGeometry", "Path")
        obj.setEditorMode("PathGeometry", 1)  # hide
    ...
    def _buildProfileOpenEdges(self, obj, edgeList, isHole, start, getsim):
        """_buildProfileOpenEdges(obj, edgeList, isHole, start, getsim) ... internal function."""
        PathLog.track()
    ...
    def _buildProfilePath_orig(self, obj, solid, isHole, start):
        ...
        depths = [obj.StartDepth.Value] + [float(d) for d in self.depthparams]
        print(f"depths: {depths}")

        slices = Macro_Slice_Solid.sliceSolid(solid, depths[:2], output="CrossSections")
        ...
        for targetFace in slices:
            pathShape = Generator_Profile.generatePathGeometry(
                targetFace,
                offset,
            ...
            for dep in depths[1:]:
                ...
                paths = Generator_Profile.geometryToGcode(
                    ...
                    dep,  # obj.FinalDepth.Value,
                )
                commands.extend(paths)

        return commands, None

    def _buildProfilePath(self, obj, solid, isHole, start):
        PathLog.info("_buildProfilePath()")
        import Macros.Generator_Profile as Generator_Profile
    ...
    def _getSolidSlices(self, obj, solid):
        ...
        depths = [obj.StartDepth.Value] + [float(d) for d in self.depthparams]
        # depths = [float(d) for d in self.depthparams]
        # print(f"depths: {depths}")

        return (
            Macro_Slice_Solid.sliceSolid(solid, depths, output="CrossSections"),
            depths,
        )

    def getRemovedMaterial(self, obj):
        removalShapes = []
        toolDiameter = self.radius * 2.0
        # pathGeomList = obj.PathGeometry.SubShapes
        pathGeomList = obj.PathGeometry.Wires
        depths = [obj.StartDepth.Value] + [float(d) for d in self.depthparams]
        ...
        for idx, btm in bottomList:
            ...
            if top is not None:
                ...
                pathArea = Macro_REST_Milling.wiresToPathFace(comp.Wires, toolDiameter)
        ...
        if removalShapes:
            rs = Part.makeCompound(removalShapes)
            Part.show(rs, "RemovedMaterial")
            return rs

        return None

    def getRestShape(self, obj):
    ...
    def opExecute(self, obj, getsim=False):
        ...
        for shape, isHole, sub in shapes:
            ...
            try:
                if profileEdgesIsOpen:
                    ...
                    if sub == "Profile":
                        (pp, sim, pathGeomList) = self._buildProfilePath(
                            obj, shape, isHole, start
                        )
                        pathGeom.extend(pathGeomList)
                    else:
                ...
                if profileEdgesIsOpen or sub in ["Profile"]:
                    ppCmds = pp
                else:
                    ppCmds = pp.Commands
        ...
        if pathGeom:
            obj.PathGeometry = Part.makeCompound(pathGeom)
        if removalShapes:
