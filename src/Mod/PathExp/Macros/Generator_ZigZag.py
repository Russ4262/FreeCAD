...
__title__ = "ZigZag Geometry and Path Generator"
...
def _linkRectangular(upHeight, trgtX, trgtY, downHeight):
    ...
    # Rapid lateral move
    paths.append(Path.Command("G0", {"X": trgtX, "Y": trgtY, "F": RAPID_HORIZ}))
    paths.append(
        Path.Command("G1", {"Z": downHeight, "F": FEED_VERT})
...
def _buildZigZagPaths(
    ...
    for wire in wireList:
        ...
        if lastPnt:
            if (
                keepToolDown
                and lastPnt.sub(pnt0).Length < keepToolDownThreshold
                and GenUtils.isMoveInRegion(
                    2.0 * toolRadius, TARGET_REGION, lastPnt, pnt0, maxWidth=0.0002
                )
            ):
                paths.append(
                    Path.Command(
            ...
            paths.append(Path.Command("G0", {"Z": retractHeight, "F": FEED_VERT}))
            paths.append(
                Path.Command(
                    "G0",
                    {
...
def geometryToGcode(
    ...
    if PATH_TYPE == "2D" and keepToolDown:
        commandList = _buildZigZagPaths(
            lineGeometry,
            retractHeight,
            finalDepth,
            keepToolDown,
...
def _getUserInput():
    ...
    guiInput.setWindowTitle("ZigZag Pattern Settings")
