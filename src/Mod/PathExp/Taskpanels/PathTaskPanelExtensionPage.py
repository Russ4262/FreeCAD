...
def rotateShapeWithVector(shape, rotVect, cor):
    print("rotateShapeWithVector()")
    rotated = shape.copy()
    if not PathGeom.isRoughly(rotVect.x, 0.0):
        rotated.rotate(cor, FreeCAD.Vector(1.0, 0.0, 0.0), rotVect.x)
    if not PathGeom.isRoughly(rotVect.y, 0.0):
        rotated.rotate(cor, FreeCAD.Vector(0.0, 1.0, 0.0), rotVect.y)
    if not PathGeom.isRoughly(rotVect.z, 0.0):
        rotated.rotate(cor, FreeCAD.Vector(0.0, 0.0, 1.0), rotVect.z)
    return rotated


def rotateShapeWithList(shape, rotations, cor):
    rotVects = {
    ...
    for axis, angle in rotations:
        rotated.rotate(cor, rotVects[axis], angle)
    return rotated
...
class TaskPanelExtensionPage(PathTaskPanelPage.TaskPanelPage):
    ...
    def getForm(self):
        ...
        uiFilePath = (
            ...
            + "Mod\\PathExp\\GuiSupport\\PageExtensionsEdit.ui"
        )
    ...
    def updateQuantitySpinBoxes(self, index=None):
        PathLog.debug("updateQuantitySpinBoxes()")
    ...
    def createItemForBaseModel(self, base, sub, edges, extensions):
        ...
        subHasItem = False
        # self.rotationCache = {}  # Reset base shape cache
    ...
    def _cachedExtension(self, obj, base, sub, label):
        ...
        if base.Name not in self.rotationCache.keys():
            opTaskPanel = self.parent.featurePages["Operation"]
            if hasattr(opTaskPanel, "rotatedShapes"):
                # print(f"rotatedShapes.keys(): {opTaskPanel.rotatedShapes.keys()}")
                self.rotationCache[base.Name] = opTaskPanel.rotatedShapes[base.Name]
            elif hasattr(obj.Proxy, "rotateShapeWithList"):
                rotations = obj.Proxy._getRotationsList(obj)
                self.rotationCache[base.Name] = obj.Proxy.rotateShapeWithList(
                    base.Shape, rotations
                )
            else:
                self.rotationCache[base.Name] = base.Shape.copy()

        # Part.show(self.rotationCache[base.Name], "rotationcache")
        if hasattr(obj, "RotationsValues") and hasattr(obj, "CenterOfRotation"):
            useRotation = True
...
class _Extension(object):
    ...
    def __init__(self, obj, base, face):
        ...
        self.rotations = []
        self.cor = FreeCAD.Vector(0.0, 0.0, 0.0)
        self.switch = None
        self.root = None
    ...
    def setEdge(self, edge):
        if edge:
            ...
            self.ext = PathFeatureExtensions.createExtension(
                self.obj, self.base, self.face, edge
            )
            self.ext.rotatedBaseShp = self.rotatedBaseShp

        self.switch = self.createExtensionSoSwitch(self.ext)
        self.root = self.switch

    def createExtensionSoSwitch_original(self, ext):
        if not ext:
            return None
        ...
        numVert = []  # track number of vertices in each polygon face
    ...
    def createExtensionSoSwitch(self, ext):
        ...
        if isinstance(wire, (list, tuple)):
            rotWire0 = rotateShapeWithList(wire[0], negRotVect, self.cor)
            rotWire1 = rotateShapeWithList(wire[1], negRotVect, self.cor)
            p0 = [p for p in rotWire0.discretize(Deflection=0.02)]
            p1 = [p for p in rotWire1.discretize(Deflection=0.02)]
            p2 = list(reversed(p1))
            polygon = [(p.x, p.y, p.z) for p in (p0 + p2)]
        else:
            if ext.extFaces:
                ...
                for face in extFaces:
                    f = rotateShapeWithList(face, negRotVect, self.cor)
                    pCnt = 0
                    wCnt = 0
                    for w in f.Wires:
                        if wCnt == 0:
                ...
                rotWire = rotateShapeWithList(wire, negRotVect, self.cor)
                # poly = [p for p in wire.discretize(Deflection=0.02)][:-1]
                poly = [p for p in rotWire.discretize(Deflection=0.02)]
                polygon = [(p.x, p.y, p.z) for p in poly]
        crd.point.setValues(polygon)
