...
class TaskPanelHeightsDepthsPage(PathTaskPanelPage.TaskPanelPage):
    ...
    def __init__(self, obj, features):
        ...
        self.OpIcon = ":/icons/Path_Stop.svg"
        self.setIcon(self.OpIcon)
        self.refImagePath = (
            f"{FreeCAD.getHomePath()}Mod\\Path\\Images\\Ops\\Path-DepthsAndHeights.gif"
        )
        self.refImage = QtGui.QPixmap(self.refImagePath)
    ...
    def getForm(self):
        # return FreeCADGui.PySideUic.loadUi(":/panels/PageRotationReferenceEdit.ui")
        return FreeCADGui.PySideUic.loadUi(
            FreeCAD.getUserAppDataDir()
            + "Mod\\PathExp\\GuiSupport\\PageRotationReferenceEdit.ui"
        )
