...
HeightsDepthsPage = LazyLoader(
    "Taskpanels.PathTaskPanelHeightsDepthsPage",
    globals(),
    "Taskpanels.PathTaskPanelHeightsDepthsPage",
)
DiametersPage = LazyLoader(
    "Taskpanels.PathTaskPanelDiametersPage",
    globals(),
    "Taskpanels.PathTaskPanelDiametersPage",
)
ExtensionPage = LazyLoader(
    "Taskpanels.PathTaskPanelExtensionPage",
    globals(),
    "Taskpanels.PathTaskPanelExtensionPage",
)
...
class TaskPanel(object):
    ...
    def _initOpAndFeaturePages(self, opPage, obj):
        """_initOpAndFeaturePages(opPage, obj) ...
        Compile and initialize the operation page and required feature pages.
        The order of pages here currently dictates the order in which the pages
        are initialized/loaded.  If one page is dependent upon data in another,
        set the correct dependency order here.
        The 'Operation' page is listed first, as it will contiain prepatory rotational
        data to be applied before the 'Extensions' page is loaded.
        """
    ...
    def _applyLayoutToPages(self, opPage, obj):
        ...
        # Make 'Operation' page last in layout
        pageNames = [k for k in self.featurePages.keys() if k != "Operation"] + [
            "Operation"
        ]
        featurePages = [self.featurePages[name] for name in pageNames]
    ...
    def accept(self, resetEdit=True):
        ...
        if self.isDirty():
            # PathLog.debug("accept() self.isDirty()")
            self.panelGetFields()
        self.preCleanup()
        self.preCleanupPreview()
    ...
    def preCleanupPreview(self):
        # Cleanup working shape previews
        page = self.featurePages["Operation"]
        if not page.targetShapeList or not self.switch:
            return
    ...
    def panelSetFields(self):
        ...
        PathLog.track()
        PathLog.debug("panelSetFields() 1")
        self.obj.Proxy.sanitizeBase(self.obj)
        for page in self.featurePages.values():
            page.pageSetFields()
        # self.obj.Proxy.printDepthParams(self.obj)
        PathLog.debug("panelSetFields() 2")
