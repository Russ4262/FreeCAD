...
class Target3DShape:
    ...
    def _debugMsg(self, msg):
        ...
        if self.isDebug:
            ...
            FreeCAD.Console.PrintMessage(
                "Target3DShape.Working3DFaces: " + msg + "\n"
            )
        else:
            PathLog.debug(msg)
        ...
        if name not in self.fullBaseDict.keys():
            ...
            if self.projectionOnly:
                bcs = PathTargetBuildUtils.getBaseCrossSection(
                    self.baseObj.Shape, self.processInternals
                )
                if bcs:
                    allFacesFused = PathTargetBuildUtils.fuseShapes(bcs.Faces)
    ...
    def _processFaces(self):
        ...
        if self.projectionOnly:
            ...
            for f in self.nonVerticalFaces:
                try:
                    csf = PathTargetBuildUtils.getCrossSectionFace(f)
                    allFaces.append(csf)
                except:
                    self._debugMsg("_processFaces() unable to cross-section face")
            if allFaces:
                allFacesFused = PathTargetBuildUtils.fuseShapes(allFaces)
                self.rawAreas.extend(allFacesFused.Faces)
                self._addDebugObject(allFacesFused, "allFacesFused")
            return
    ...
    def getWorkingSolids_OLD(self, avoidOverhead=False):
        """getWorkingSolids()...
        This is the main class control function for identifying pocket areas,
        including extensions to selected faces using input from the Base Geometry
        and Extensions features.
