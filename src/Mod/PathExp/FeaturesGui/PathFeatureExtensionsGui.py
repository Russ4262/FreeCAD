...
class _Extension_orig(object):
    ...
    def createExtensionSoSwitch(self, ext):
        ...
        if isinstance(wire, (list, tuple)):
            w0 = _applyShapeRotation(wire[0], ext.rotationsApplied)
            w1 = _applyShapeRotation(wire[1], ext.rotationsApplied)
            p0 = [p for p in w0.discretize(Deflection=0.02)]
            p1 = [p for p in w1.discretize(Deflection=0.02)]
            p2 = list(reversed(p1))
...
class _Extension(object):
    ...
    def setEdge(self, edge):
        if edge:
            self.edge = edge
            self.ext = FeatureExtensions.createExtension(
...
class TaskPanelExtensionPage(PathOpGui.TaskPanelPage):
    ...
    def _cachedExtension(self, obj, base, sub, label):
        ...
        if base.Name not in self.rotationCache.keys():
            rotations, __ = obj.Proxy.getRotations(obj)
            self.rotationCache[base.Name] = obj.Proxy.rotateShape(base.Shape, rotations)
