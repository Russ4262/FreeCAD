...
class Target2DArea:
    ...
    def __init__(
        ...
        self.avoidRegions = list()
        self.extensionFaces = list()
        self.allOuters = list()
        self.workingHoles = list()
        self.workingAreaTups = list()
        self.workingHoleTups = list()
        self.targetAreaTups = list()
        self.targetHoleTups = list()
        self.targetOpenEdgeTups = list()
        self.targetSolidTups = list()  # Empty place holder
    ...
    def _debugMsg(self, msg, isError=False):
        ...
        """
        if isError:
            FreeCAD.Console.PrintError("Target2DArea: " + msg + "\n")
            return

        if self.isDebug:
            # PathLog.info(msg)
            FreeCAD.Console.PrintMessage("Target2DArea: " + msg + "\n")
        else:
            PathLog.debug(msg)
        ...
        if type(face.Surface) == Part.Cylinder or type(face.Surface) == Part.Cone:
            if not isVertical(face.Surface.normal(0, 0)):
                ...
                if csFace:
                    ...
                    self._debugMsg("_clasifyFace() no csFace")
                    return False
        ...
        if wires:
            for w in wires:
                if w.isClosed():
                    ...
                    targetOpenEdgeTups.append(
                        (self.baseObj, w)
                    )  # Later processing of open edges in Profile module
        else:
            self._debugMsg("Working2DAreas: No wires from flattened edges.")

        if targetOpenEdgeTups:
            self.targetOpenEdgeTups.extend(targetOpenEdgeTups)
    ...
    def _processVertFaces(self):
        ...
        if wires:
            for w in wires:
                ...
                self.targetOpenEdgeTups.append(
                    (self.baseObj, w)
                )  # Later processing of open edges in Profile module
    ...
    def _applyBoundaryShape(self):
        ...
        if self.boundaryShape == "Boundbox":
            ...
            # Two methods for getting outline: TechDraw.findShapeOutline and self._get_PathTargetBuildUtils.getCrossSectionFace
    ...
    def _processExtensions(self):
        ...
        for ext in self.extensions:
            ...
            if (
                ext.obj.Name == self.baseObj.Name
                and ext.feature in self.subs[self.baseObj.Name]
            ):
                wire = ext.getWire()
                if wire:
                    # process extension if an avoided face
                    if ext.avoid:
                        for f in ext.getExtensionFaces(wire):
                            self.avoidRegions.append(f)
    ...
    def _identifyWorkAreas(self):
        ...
        # Place all faces into same working plane
        for h in self.avoidRegions:
            h.translate(FreeCAD.Vector(0.0, 0.0, 0.0 - h.BoundBox.ZMin))

        # Second face-combining method attempted
        chf = PathTargetBuildUtils.combineHorizontalFaces(
            self.allOuters, self.avoidRegions
        )
        if chf:
            ...
            print("No combineHorizontalFaces() returned")
    ...
    def _getBaseCrossSection(self, base):
        self._debugMsg("_getBaseCrossSection()")
        ...
        if self.processInternals:
            ...
            bcs = PathTargetBuildUtils.getCrossSectionFace(base.Shape)
        self.basePerimeterDict[base.Name] = bcs
        return bcs

    def _getHoleAreas(self):
        """_getHoleAreas()... Returns list of holes (any internal openning in face) identified"""
        if self.processInternals and self.workingHoles:
            if self.isDebug:
    ...
    def _saveData_ORIG(self, base):
        for a in self.workingAreas:
            self.targetAreaTups.append((base, a))
        for h in self._getHoleAreas():
            self.targetHoleTups.append((base, h))
        self.workingAreas = list()
        self.workingHoles = list()

    def _saveData(self, base):
        areas = Part.makeCompound(self.workingAreas)
        holes = Part.makeCompound(self._getHoleAreas())
        targetAreas = areas.cut(holes)

        for f in targetAreas.Faces:
            self.targetAreaTups.append((base, f))
        self.workingAreas = list()
        self.workingHoles = list()
    ...
    def applyMaterialAllowance(self, materialAllowance):
        ...
        if materialAllowance == 0.0:
            # Do nothing for shrinking target area
            return

        # self.finalDepth
        targetAreaTups = list()

        # Trim target face using base shape when material allowance is positive
        for base, face in self.targetAreaTups:
            if materialAllowance > 0.0:
                orah = PathTargetBuildUtils.getOverheadRegionsAboveHeight(
                    base.Shape, self.finalDepth
                )
                offsetOrah = orah
                offsetFace = PathUtils.getOffsetArea(face, materialAllowance)

                if not offsetOrah or not offsetFace:
                    self._debugMsg("getOffsetArea() failed")
                    targetAreaTups.append((base, face))
                elif len(offsetOrah.Faces) == 0 or len(offsetFace.Faces) == 0:
                    self._debugMsg("No offset faces.")
                    targetAreaTups.append((base, face))
                else:
                    useFace = offsetFace.cut(offsetOrah)
                    targetAreaTups.append((base, useFace))
            else:
                offsetFace = PathUtils.getOffsetArea(face, materialAllowance)

                if not offsetFace:
                    self._debugMsg("getOffsetArea() failed", isError=True)
                    # targetAreaTups.append((base, face))
                elif len(offsetFace.Faces) == 0:
                    self._debugMsg(
                        "No face remaining after Material Allowance applied.",
                        isError=True,
                    )
                    # targetAreaTups.append((base, face))
                else:
                    targetAreaTups.append((base, offsetFace))

        self.targetAreaTups = targetAreaTups
    ...
    def buildTargetShapes(self, avoidOverhead=False):
        ...
        if not self.baseObjectList:
            self._debugMsg("No source base objects")
            return
    ...
    def getWorkingAreas_OLD(self, avoidOverhead=False):
        """getWorkingAreas(avoidOverhead=False)...
        This is the main class control function for identifying pocket areas,
        including extensions to selected faces using input from the Base Geometry
        and Extensions features.
        """
        ...
        if self.workingAreaTups:
            return self.workingAreaTups
        print("No workingAreaTups")

        if self.processInternals and self.workingHoleTups:
            return self.workingHoleTups

        print("No workingHoleTups")
    ...
    def buildOpenEdges(
        ...
        """buildOpenEdges(toolRadius, offsetRadius, jobTolerance, jobLabel='Job')...
        ...
        """
        self._debugMsg("buildOpenEdges()")

        for (base, wire) in self.targetOpenEdgeTups:
            ...
            if openEdges:
                self.targetOpenEdges.extend(openEdges)

        return self.targetOpenEdges
